               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    // start convolution
    #pragma omp parallel for firstprivate(outPtr,inPtr, kPtr) schedule(static,dataSizeY/4)
    for(i= 0; i < dataSizeY; ++i)                   // number of rows
    {
        //printf("Hello World from thread = %d // i = %d \n", omp_get_thread_num(),i);
        //printf("thread %d\n", omp_get_thread_num());
        int rowMin=0, rowMax=0, j, colMax,colMin;

        // compute the range of convolution, the current row of kernel should be between these
        //protect outbounds and fix rows 
        if ((i - dataSizeY + kCenterY + 1) > 0)
        {
            rowMin = i - dataSizeY + kCenterY + 1;
        }
        
        if ((kernelSizeY-1) < (i+kCenterY))
        {
            rowMax = kernelSizeY-1;
        }else{
            rowMax = i+kCenterY;
        }
        ...
        for(j = 0; j < dataSizeX; ++j)              // number of columns
        {
            ...
            //protect outbounds col and fix blocks
            if (j+kCenterX< kernelSizeX-1)
            {
                colMax = j+kCenterX;
            }else{
                colMax = kernelSizeX -1;
            }

            if (j - dataSizeX + kCenterX + 1> 0)
            {
                colMin = j - dataSizeX + kCenterX + 1;
            }else{
                colMin = 0;
            }

            inPtr = inPtr2 + (i*dataSizeX) + j;// Next input
            
            // flip the kernel and traverse all the kernel values
            // multiply each kernel value with underlying input data
            for(m = rowMin; m <= rowMax; ++m)        // kernel fixed rows
            {

                    int *aux = inPtr - m * dataSizeX;///// Aux var block input

                    for(n = colMin; n <= colMax; ++n) // kernel fixed col
                    {
                        kPtr = kernel + (m * kernelSizeX) + n;// next kernel 
                        sum += *(aux - n) * *kPtr;                     
                    }

            }
            outPtr = out + (i*dataSizeX) + j;//next block output
